    ...
    def get_start_end(self,file_containing_start_pattern=sys.argv[4]):
            ...
            if len(dir) != 0: 
                ...
                if demarcation_value_in_file:
                    if demarcation_value_in_file.startswith('iqtrace'):
                        ...
                        demarcation_found=metadata_value
                        date_tz_file=fname.split('_')[1]
                        time_tz_file=fname.split('_')[2].split('.')[0]
                            ...
                            for date_object in dates_range:
                                for file in os.listdir(output):
                                    if file.strip().startswith('iqtrace'):
                                        if date_object.strftime("%d-%m-%Y")  == file.split('_')[1]:
                                            if file.split('_')[1] == date_tz_file and file.split('_')[2].split('.')[0] < time_tz_file or file.split('_')[1] < date_tz_file:
                                                continue
                                            else:
                                                #print('extract'+file)
        ...
        if not found:
            ...
            if metadata_value.endswith('_demarcation_1'):
                if not os.path.exists('/var/tmp/parse_tmp/first_occurance_instance_one'):
                    with open(file_containing_start_pattern.strip()) as infile, open(outputfile, 'a') as outfile:
                        ...
                        for line in infile:
                            if self.start_found in line:
                                ...
                                copy = False
                                outfile.write(line)
                                open('/var/tmp/parse_tmp/first_occurance_instance_one', 'a').close() # touch file. as execute only 1st time
                                new=outputfile[:-1]+'0'
            ...
            for _lines_ in metadata_file.readlines():
                ...
                for _line_1_ in select_unique:
                    ...
                    if any(k in _line_1_ for k in select_star) or select_top:
                        ...
                        for ch in ['distinct ','"','sum(','dc.','dim.','*','as ','\'','if(','if ','count ','results ','result ','and ','min(','max(','avg(','dcpublic.','dcbo.','(',')']:
                            if ch in rawList:
                                if ch == "*":
                                    ...
                                    if all(k in item.strip() for k in keywords):
                                        ...
                                        for alias_name in space_sep_alias:
                                            if alias_flag == 1:
                                                ...
                                                if any(t in alias_name for t in _tables_):
...
class User_Display:
    ...
    def user_display(self):
        ...
        with open(unused_counter_list_file, "r+") as f: s = f.read(); f.seek(0); f.write("# Table_Name::Counter_Name::Feature_Name #\n" + s)
...
if __name__== "__main__":
    ...
    if parsing_level_type == "parsing_level_1":
        ...
        parallel_threads=counter_tool_work_dir+"/parallel_threads"
        ...
        #cleanup
        for files in [unused_counter_list_file,counter_data_per_date,aggregated_counters]:
            if os.path.exists(files):
                os.remove(files)
